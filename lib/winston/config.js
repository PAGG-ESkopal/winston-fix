...
config.colorize = function (level, message) {
  // if someone calls colorize -- force colors to colorize the message
  //  even if the local hardware doesn't support it
  colors.enabled = true;
  ...
  if (allColors[level] instanceof Array) {
    for (var i = 0, l = allColors[level].length; i < l; ++i) {
      if (colors[allColors[level][i]] !== 'none') {
